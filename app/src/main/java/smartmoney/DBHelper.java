...
{
    private static final String DATABASE_NAME = "GoalWatch.db";
     ...
     * false otherwise
     */
    public boolean insertGoal(final String name, final int max)
    {
        SQLiteDatabase db = getWritableDatabase();
        boolean result = insertGoal(db, name, max);
        db.close();
     ...
     * false otherwise
     */
    public boolean insertGoal(SQLiteDatabase writableDb, final String name, final int max)
    {
     ...
     */
    public boolean updateGoal(final String name, final int max)
    {
        ContentValues contentValues = new ContentValues();
        contentValues.put(BudgetDbIds.MAX, max);
     ...
     * false otherwise
     */
    public boolean deleteGoal (final String name)
    {
        SQLiteDatabase db = getWritableDatabase();
    ...
    /**
     * Get Goal object for the named budget in the database,
     * but only fill in the 'name' and 'max' fields;
     * the value of other fields is undefined.
     *
     * @param name
     *      name of the budget to query
     * @return Goal object representing the named budget,
     * or null if it could not be queried
     */
    public Goal getGoalStoredOnly(final String name)
    {
        ...
        if(data.getCount() == 1)
        {
            ...
            budget = new Goal(goalName, goalMax, 0);
        }
    ...
    public Goal getGoal(String budget) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor data = db.rawQuery("select * from " + BudgetDbIds.TABLE +
                " where " + BudgetDbIds.NAME + "=?", new String[]{budget});
        ...
        if(data.getCount() == 1)
        {
            ...
            budgets = new Goal(goalName, goalMax, 0);
        }
    ...
    /**
     * Get Goal objects for each budget in the database,
     ...
     *      into the 'current' field.
     * @return list of Goal objects, or an empty field if none
     * could be found.
     */
    public List<Goal> getGoals(long startDateMs, long endDateMs)
    {
        SQLiteDatabase db = getReadableDatabase();
        ...
        LinkedList<Goal> budgets = new LinkedList<>();
        ...
        if(data.moveToFirst())
        {
            do
            {
                ...
                double current = revenues - expenses;
                int currentRounded = (int)Math.ceil(current);

                budgets.add(new Goal(name, max, currentRounded,expenses,revenues));
    ...
    /**
     * Get Goal object representing transactions which
     ...
     * @return Goal object
     */
    public Goal getBlankGoal(long startDateMs, long endDateMs)
    {
        SQLiteDatabase db = getReadableDatabase();
